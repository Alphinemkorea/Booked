...
import { Search, ShoppingBag, Library, Moon, Sun, ChevronDown, Command } from 'lucide-react';
        ...
        <div className="container" style={{ height: '100%', display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginLeft: 'auto' }}>
            ...
            <button
              ...
              style={{ position: 'relative', width: 42, height: 42, border: 'none', background: 'transparent', borderRadius: 12, cursor: 'pointer', color: 'var(--ink)' }}
            >
              <ShoppingBag size={20} />
              {purchase.length > 0 && (
                <span style={{ position: 'absolute', top: 4, right: 4, minWidth: 18, height: 18, borderRadius: 999, background: 'var(--primary)', color: '#fff', fontSize: 11, fontWeight: 800, display: 'grid', placeItems: 'center' }}>{purchase.length}</span>
              )}
            </button>
            <button
              type="button"
            ...
            >
              <Library size={20} />
              {lending.length > 0 && (
                <span style={{ position: 'absolute', top: 4, right: 4, minWidth: 18, height: 18, borderRadius: 999, background: 'var(--primary)', color: '#fff', fontSize: 11, fontWeight: 800, display: 'grid', placeItems: 'center' }}>{lending.length}</span>
              )}
            </button>
            {user ? (
              <div style={{ position: 'relative' }}>
                ...
                >
                  <span style={{ width: 32, height: 32, borderRadius: '50%', background: 'rgba(255,255,255,.2)', display: 'grid', placeItems: 'center' }}>
