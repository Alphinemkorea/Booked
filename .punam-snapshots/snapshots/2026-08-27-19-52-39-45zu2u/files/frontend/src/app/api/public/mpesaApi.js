 ...
 * Calls BOOKED backend (/api/mpesa/*) — never talks to Daraja directly.
 */
 ...
 */
export async function initiateStkPush({ phone, amount, accountRef = 'BOOKED', description }) {
  ...
  try {
    const data = await api('/api/mpesa/stk', {
      ...
      body: JSON.stringify({
        ...
        accountRef,
        description: description || 'BOOKED payment',
      }),
    });
