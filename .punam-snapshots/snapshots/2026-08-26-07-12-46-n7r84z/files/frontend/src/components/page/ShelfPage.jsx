    ...
    <div className="container" style={{ padding: '36px 0 56px', minHeight: '50vh' }}>
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 12 }}>
        {tab === 'purchases' && (myOrders.filter((o) => o.status !== 'paid' && o.status !== 'rejected').length === 0 ? (
          <EmptyState title="No purchases yet" description="When you order books, they appear here until you pay." action={<Link to="/shop" className="btn btn-outline">Browse Shop</Link>} />
        ) : myOrders.filter((o) => o.status !== 'paid' && o.status !== 'rejected').map((o) => (
          <article key={o.id} className="card" style={{ padding: 18 }}>
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 10, alignItems: 'center' }}>
              <span className="chip">#{o.id}</span><StatusChip status={o.status} /><strong className="price">{formatKES(o.total)}</strong>
            </div>
        ...
        {tab === 'borrowing' && (myLoans.filter((l) => ['pending', 'active', 'return_requested'].includes(l.status)).length === 0 ? (
          <EmptyState title="No active loans" description="Borrow from the library — requests show here after you submit." action={<Link to="/library" className="btn btn-outline">Browse Library</Link>} />
        ...
        {tab === 'history' && (
          <>
            {myOrders.filter((o) => o.status === 'paid' || o.status === 'rejected').map((o) => (
              <article key={o.id} className="card" style={{ padding: 18 }}>
                <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}><span>Purchase · {o.items[0]?.title}</span><StatusChip status={o.status} /><strong className="price">{formatKES(o.total)}</strong></div>
            ...
            {myLoans.filter((l) => l.status === 'returned' || l.status === 'rejected').map((l) => (
              <article key={l.id} className="card" style={{ padding: 18 }}><span>Loan · {l.title}</span> <StatusChip status={l.status} /></article>
            ))}
            {myOrders.filter((o) => o.status === 'paid' || o.status === 'rejected').length + myLoans.filter((l) => l.status === 'returned' || l.status === 'rejected').length === 0 && <EmptyState title="No history yet" description="Completed purchases and returned loans will live here." />}
          </>
        )}
      </div>
      {payId && (
