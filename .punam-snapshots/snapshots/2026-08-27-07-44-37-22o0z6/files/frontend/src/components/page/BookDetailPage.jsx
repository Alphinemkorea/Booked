...
import { Heart, Zap, Shield, BookOpen, Smartphone } from 'lucide-react';
...
export function BookDetailPage() {
  ...
  const access = book ? resolveBookAccess(book.id, { user, purchases, loans }) : { canRead: false };
  const wished = book ? wish.includes(book.id) : false;
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(240px, 340px) 1fr', gap: 48, alignItems: 'start' }}>
        <div style={{ background: 'var(--well)', borderRadius: 16, padding: 28, position: 'sticky', top: 'calc(var(--nav-h) + 16px)' }} className="hide-mobile">
          ...
          <button
            ...
            style={{ marginTop: 16 }}
            onClick={() => navigate(`/read/${book.id}?sample=1`)}
          >
            Read sample
          </button>
        </div>
        ...
        <div>
          ...
          <div className="access-grid">
            <div className="access-item">
              <strong><Zap size={14} style={{ verticalAlign: -2 }} /> Instant open</strong>
              <p>Pages load in your browser right after payment.</p>
            </div>
            <div className="access-item">
              <strong><Smartphone size={14} style={{ verticalAlign: -2 }} /> Read anywhere</strong>
              <p>Phone, tablet, or laptop — no app install required.</p>
            </div>
            <div className="access-item">
              <strong><Shield size={14} style={{ verticalAlign: -2 }} /> Clear terms</strong>
              <p>Own forever, or borrow until the due date locks access.</p>
            </div>
          </div>

          {access.canRead && (
            <Link to={`/read/${book.id}`} className="btn btn-primary" style={{ marginBottom: 16 }}>
          ...
          >
            <div>
              ...
              <p style={{ margin: '0 0 10px', fontSize: 14, color: 'var(--muted)', lineHeight: 1.45 }}>Own the digital edition forever. Read in your browser anytime — no shipping.</p>
            ...
            <div>
              ...
              <p style={{ margin: '0 0 10px', fontSize: 14, color: 'var(--muted)', lineHeight: 1.45 }}>Digital loan for {book.loanDays} days. Access locks when time ends. Deposit refunded on return.</p>
      ...
      <div className="sticky-cta hide-desktop">
        {book.forSale && (
          <button type="button" className="btn btn-primary" style={{ flex: 1 }} disabled={book.stock < 1} onClick={buy}>
            Own digital · {formatKES(book.price)}
          </button>
        )}
        {book.forLoan && (
          <button type="button" className="btn btn-outline" style={{ flex: 1 }} onClick={borrow}>
            Borrow · {book.loanDays || 14}d
          </button>
        )}
        <button type="button" className="btn btn-ghost" style={{ flex: '0 0 auto' }} onClick={() => navigate(`/read/${book.id}?sample=1`)}>
          Sample
        </button>
      </div>
