...
export function ShelfPage() {
  ...
  const onPaySuccess = ({ receipt, msisdn }) => {
    if (payingOrder) {
      ...
      dispatch(pushToast({ message: `Paid · open your digital library · ${receipt}` }));
    }
    if (payingLoan) {
      ...
      <div style={{ maxWidth: 720, margin: '0 auto' }}>
        ...
        {tab === 'borrowing' && (
          myLoans.filter((l) => ['pending', 'approved', 'active', 'return_requested'].includes(l.status)).length === 0 ? (
            <EmptyState
              title="No open loans"
              description="Request from the library → admin approves → pay deposit → read online until due date."
