  ...
  if (!access.canRead) {
    return (
      <div className="container empty-state" style={{ padding: 48, textAlign: 'center' }}>
        ...
        {access.label?.includes('deposit') && (
          <Link to="/shelf?tab=borrowing" className="btn btn-primary">Pay deposit on Shelf</Link>
        )}
        {!access.label?.includes('deposit') && access.label?.includes('Buy') && (
  ...
  return (
    <div style={{ minHeight: '100vh', background: t.bg, color: t.ink, display: 'flex', flexDirection: 'column' }}>
      ...
      >
        <button
          type="button"
          onClick={() => navigate(params.get('from') || '/shelf')}
          aria-label="Close"
          style={{ width: 36, height: 36, borderRadius: 10, border: '1px solid var(--line)', background: 'var(--card)', cursor: 'pointer', display: 'grid', placeItems: 'center' }}
        >
          <X size={18} />
        </button>
      ...
      {access.mode === 'borrowed' && (
        <div style={{ textAlign: 'center', padding: '8px 16px', fontSize: 13, fontWeight: 700, background: 'var(--warning-soft)', color: 'var(--warning)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
          <BookOpen size={14} />
      ...
      <article style={{ flex: 1, width: 'min(680px, calc(100% - 40px))', margin: '32px auto 100px', fontSize: font, lineHeight: 1.75 }}>
        ...
        {current.body.split('\n').map((line, i) =>
          line.trim() === '' ? <br key={i} /> : <p key={i} style={{ margin: '0 0 1.1em', fontFamily: 'var(--font-serif)', fontWeight: 500 }}>{line}</p>
        )}
      </article>
